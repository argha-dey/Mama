...
class _PollingQstnScreenState extends State<PollingQstnScreen> {
  ...
  List<PollingQuestion> pollQstnList = [];
  List<PollingQuestion> pollQstWithAnsnList = [];
  List<PollingAnsDetails> pollingAnsList = [];
    ...
    try {
      ...
      if (response.statusCode == 200) {
        setState(() {
          ...
          if (!pollingQstnDetailsModel!.status!) {
            global.showAlertDialog(
                "Alert", pollingQstnDetailsModel!.massage.toString(), context);
          }
        });
      } else {
            ...
            children: [
              ...
              Visibility(
                  ...
                  child: InkWell(
                    ...
                    onTap: () {
                      if (pollingQstnDetailsModel!.data!.pollingHour! < 1) {
                        global.showAlertDialog(
                            "Alert", "This Poll is expire", context);
                      } else {
  ...
  dynamic submitQusAns() async {
    // show loader
    pollingAnsList = [];
    for (int i = 0; i < selectedValues.length; i++) {
      pollingAnsList.add(new PollingAnsDetails(
          widget.pollingId.toString(),
          PrefObj.preferences!.get(PrefKeys.USER_ID).toString(),
          pollQstnList[i].pollingQuestionId.toString(),
          selectedValues[i].toString()));
    }

    global().showLoader(context);
    final http.Response response =
    await repository.onPollingAnsSubmitApi(pollingAnsList);
    ...
    if (response.statusCode == 200) {
      debugPrint("Polling answer submit successfully");
      Navigator.push(
        context,
        MaterialPageRoute(
            builder: (context) => PollingSubmitScreen(
              pollingId: widget.pollingId.toString(),
            )),
      );
    } else {
      global.showAlertDialog("Alert", createPollingStatus.toString(), context);
    }
  }
}

class PollingAnsDetails {
  String? polling_id;
  String? user_id;
  String? polling_question_id;
  String? given_answer;

  PollingAnsDetails(this.polling_id, this.user_id, this.polling_question_id,
      this.given_answer);

  PollingAnsDetails.fromJson(Map<String, dynamic> json) {
    polling_id = json['polling_id'];
    user_id = json['user_id'];
    polling_question_id = json['polling_question_id'];
  ...
  Map<String, dynamic> toJson() {
    ...
    data['polling_question_id'] = this.polling_question_id;
